...
def _infer_source_json(csv_path: Path, source_json_arg: str) -> Path:
    if source_json_arg.strip():
        return Path(source_json_arg)

    candidates = [
        csv_path.with_suffix(".json"),
        csv_path.with_suffix(".jsonl"),
    ]
    for p in candidates:
        if p.exists():
            return p
    ...
    raise FileNotFoundError(
        f"Cannot infer source JSON for CSV: {csv_path}. Tried: {names}. "
        "Pass --source-json explicitly."
    )


def _infer_export_tag(csv_path: Path) -> str:
    day = ""
    for part in csv_path.parts:
        if re.fullmatch(r"day\d+", part, flags=re.IGNORECASE):
            day = part
            break

    stem = csv_path.stem
    owner = stem.split("_")[0] if "_" in stem else stem
...
def _infer_output_dir(csv_path: Path, output_dir_arg: str) -> Path:
    if output_dir_arg.strip():
        return Path(output_dir_arg)
    project_root = Path(__file__).resolve().parent
    tag = _infer_export_tag(csv_path)
    return project_root / "export" / tag


def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Export assignment CSV (e.g. Task/.../day08/Yu_primary_day08.csv) "
            "to input/output JSON."
        )
    )
    parser.add_argument("--input-csv", required=True, help="Path to assignment CSV.")
    parser.add_argument(
        "--source-json",
        default="",
        help="Optional source JSON/JSONL template. Defaults to sibling .json of the CSV.",
    )
    parser.add_argument(
        "--output-dir",
        default="",
        help="Output directory. Defaults to export/<dayXXName> inferred from CSV path.",
    )
    parser.add_argument(
        "--include-skipped",
        action="store_true",
        help="Include rows with abandon/skipped=True in exported_labels.json.",
    )
    ...
    source_json_path = _infer_source_json(csv_path, args.source_json)
    source_items = _load_source_items(source_json_path)
    if not source_items:
        raise RuntimeError(
            f"Source template is empty or missing valid records: {source_json_path}"
        )

    out_dir = _infer_output_dir(csv_path, args.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / "exported_labels.json"
    abandoned_json_path = out_dir / "abandoned_records.json"
